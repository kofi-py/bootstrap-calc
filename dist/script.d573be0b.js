...
for (var _i = 0, _arr = ["add", "subtract", "multiply", "divide", "percent"]; _i < _arr.length; _i++) {
  var opName = _arr[_i];
  document.querySelector(".btn-secondary[op=".concat(opName, "]")).onclick = opCallback(opName);
}
...
document.querySelector(".btn-secondary[op=clear]").onclick = function () {
  $input.value = 0;
  buffer.length = 0;
};

document.querySelector(".btn-secondary[op=negate]").onclick = function () {
