...
for (const opName of [ "add", "subtract", "multiply", "divide", "percent" ]) {
    document.querySelector(`.btn-secondary[op=${opName}]`).onclick =
      opCallback(opName);
}
  ...
  document.querySelector(".btn-secondary[op=clear]").onclick =
    () => {
      $input.value = 0;
      buffer.length = 0;
    }
  
  document.querySelector(".btn-secondary[op=negate]").onclick =
